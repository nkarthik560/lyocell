...
if (portletTitleBasedNavigation) {
	...
	if(fromCopyURL!=null &&fromCopyURL.equalsIgnoreCase("true")) {
	renderResponse.setTitle("Copy Files");
	}else{
...
<div <%= portletTitleBasedNavigation ? "class=\"container-fluid-1280\"" : StringPool.BLANK %>>
	...
	<aui:form action="<%= moveFileEntryURL %>" enctype="multipart/form-data" method="post" name="fm" onSubmit='<%= "event.preventDefault(); " + renderResponse.getNamespace() + "saveFileEntry(false);" %>'>
		...
		<aui:input name="fromCopyURL" value="<%=fromCopyURL%>" type="hidden"/>
		...
		<aui:fieldset-group markupView="lexicon">
			<aui:fieldset>
				<c:if test="<%= !validMoveFolders.isEmpty() %>">
					...
					<% if(fromCopyURL!=null &&fromCopyURL.equalsIgnoreCase("true")) { %>
					<h4><liferay-ui:message arguments="<%= validMoveFolders.size() %>" key="folders are ready to be copied" translateArguments="<%= false %>" /></h4>
					<%}else{ %>
				...
				<c:if test="<%= !invalidMoveFolders.isEmpty() %>">
					<div class="move-list-info">
					<%if(fromCopyURL!=null &&fromCopyURL.equalsIgnoreCase("true")) {%>
						<h4><liferay-ui:message arguments="<%= invalidMoveFolders.size() %>" key="folders can not to be copied" translateArguments="<%= false %>" /></h4>
					<%}else{ %>
				...
				<c:if test="<%= !validMoveFileEntries.isEmpty() %>">
					<div class="move-list-info">
					<%if(fromCopyURL!=null &&fromCopyURL.equalsIgnoreCase("true")) {%>
						<h4><liferay-ui:message arguments="<%= validMoveFileEntries.size() %>" key="files are ready to be copied" translateArguments="<%= false %>" /></h4>
					<%}else{ %>
				...
				<c:if test="<%= !invalidMoveFileEntries.isEmpty() %>">
					<div class="move-list-info">
						<%if(fromCopyURL!=null &&fromCopyURL.equalsIgnoreCase("true")) {%>
						<h4><liferay-ui:message arguments="<%= invalidMoveFileEntries.size() %>" key="files cannot be copied" translateArguments="<%= false %>" /></h4>
					   <%}else{ %>
